...
- Original: loaded only GEMINI_API_KEY, raised RuntimeError if missing
- Now: loads all provider keys optionally, exposes them via dict
- WHY: multi-provider requires graceful degradation, not hard crash
"""
...
# ---------------------------------------------------------------------------
# Provider API keys — all optional, missing = provider unavailable
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Provider model overrides — all optional, defaults are in provider adapters
# ---------------------------------------------------------------------------
...
# Comma-separated provider priority: "gemini,xai,groq,cerebras,mistral,openrouter"
AI_PROVIDER_PRIORITY: str = os.getenv(
    "AI_PROVIDER_PRIORITY",
    "gemini,xai,groq,cerebras,mistral,openrouter",
)

# Set to "mock" to use the mock provider for testing (no API calls)
AI_MODE: str = os.getenv("AI_MODE", "live")

# ---------------------------------------------------------------------------
# Convenience: all keys in one dict for the registry
# ---------------------------------------------------------------------------
