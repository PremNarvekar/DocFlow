...
export default function MockResultsViewer() {
  const mockData = {
    ...
    due_date: "2026-09-30",
    vendor_name: "Acme Corp Tech",
    customer_name: "Stark Industries",
    currency: "USD",
    items: [
      { description: "Server Rack Phase 1", quantity: 2, unit_price: 1500, amount: 3000 },
      { description: "Cloud Storage (TB)", quantity: 50, unit_price: 10, amount: 500 }
    ],
    subtotal: 3500,
    tax: 350,
    discount: 0,
      ...
      <div className="p-5 flex-1 overflow-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 mb-6">
          <div>
            ...
            <p className="text-sm font-semibold text-slate-900">{mockData.vendor_name}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Customer</p>
            <p className="text-sm font-semibold text-slate-900">{mockData.customer_name}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Invoice Number</p>
            <p className="text-sm font-medium text-slate-800">{mockData.invoice_number}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-1">Date</p>
            <p className="text-sm font-medium text-slate-800">{mockData.invoice_date}</p>
          </div>
        </div>
        ...
        <div className="flex justify-end pt-3 border-t border-slate-200">
          <div className="w-full sm:w-1/2 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-slate-800">${mockData.subtotal}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-slate-500">Tax</span>
              <span className="font-medium text-slate-800">${mockData.tax}</span>
            </div>
            <div className="flex justify-between text-base font-bold pt-2 border-t border-slate-100">
              <span className="text-slate-900">Total ({mockData.currency})</span>
              <span className="text-blue-600">${mockData.total}</span>
            </div>
          </div>
        </div>
      </div>
